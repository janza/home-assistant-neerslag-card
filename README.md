...
# Josip Rain Card
 > Recommendation: Use the `Neerslag App` or migrate from the `Josip Rain Card` to the `Neerslag App` (all-one-package: sensors + card). The Neerslag App is part of the default HACS repository.
...
2) Add the Josip Rain Card in the Home Assistant Community Store as front-end plugin.
...
1) Download the latest release of the Josip Rain Card from this repository;
2) In Home Assistant, create the folder `config/www/community` if it does not exist;
3) Add the Josip Rain Card to the `community` folder. (i.e. `community/josip-rain-card/`);
4) Add the Josip Rain Card as a resource in Home Assistant (config/lovelace/resources);
5) Using the following details as resource: `/hacsfiles/josip-rain-card/josip-rain-card.js` as Javascript module.
 ...
 * Replace `<lon-2-decimals>` with your longitude. For example: `{{state_attr("zone.home", "longitude") | round(2, default="not available")}}`
 ...
 ## 3. Adding the Josip Rain Card to your dashboard
Via the interface:
1) Go in to "Configure UI mode" on your dashboard
2) Click on "Add Cards" and find the "Josip Rain Card" in the list of cards

### Configuration options:

#### Using one sensor:
```yaml
type: 'custom:josip-rain-card'
title: Neerslag
